...
import {
    MapPinIcon, ClockIcon, UserIcon, PhoneIcon, EnvelopeIcon,
    NoteIcon, TruckIcon, BagIcon, DeviceMobileIcon,
    CreditCardIcon, MoneyIcon, CheckCircleIcon, ArrowRightIcon,
    ArrowLeftIcon, ShoppingBagIcon, PencilSimpleIcon, LockIcon,
    WarningCircleIcon, CaretDownIcon
...
// ─────────────────────────────────────────────
// ORDER SUMMARY (shared panel)
// ─────────────────────────────────────────────
function OrderSummary({ orderType, paymentMethod }: { orderType: OrderType; paymentMethod: PaymentMethod }) {
            ...
            <div className="flex flex-col gap-2 text-sm">
                ...
                <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-text-dark dark:text-text-light">
                        {orderType === 'delivery' ? formatPrice(DELIVERY_FEE) : <span className="text-secondary">Free</span>}
                    </span>
                </div>
                <div className="flex justify-between">
...
function StepDetails({
    ...
    const update = (field: keyof ContactDetails) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
        ...
        <div className="flex flex-col gap-5">
            ...
            <div className="bg-white dark:bg-brand-dark rounded-2xl p-5 flex flex-col gap-4 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-3">
                    {([
                        { type: 'delivery', icon: <TruckIcon weight="fill" size={22} />, label: 'Delivery', sub: 'Delivered to you' },
                        { type: 'pickup', icon: <BagIcon weight="fill" size={22} />, label: 'Pickup', sub: 'Pick up at branch' },
                        ...
                        <button
                            ...
                            className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all duration-150
                                ${orderType === type
                                    ...
                                    : 'border-neutral-gray/15 text-neutral-gray hover:border-primary/30'
                                }`}
                        >
                ...
                <div className="bg-white dark:bg-brand-dark rounded-2xl p-5 shadow-sm flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                        <h2 className="font-bold text-text-dark dark:text-text-light">
                            {orderType === 'delivery' ? 'Delivery Details' : 'Pickup Location'}
                        </h2>
                    ...
                    <div className="flex items-start gap-3 p-3 rounded-xl bg-neutral-light dark:bg-brown/30">
                        <MapPinIcon weight="fill" size={18} className="text-primary mt-0.5 shrink-0" />
                        <div>
            ...
            <div className="bg-white dark:bg-brand-dark rounded-2xl p-5 shadow-sm flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <FormField icon={<UserIcon weight="fill" size={15} />} label="Full Name *">
                        <input
                            type="text" placeholder="e.g. Kwame Mensah"
                            value={contact.name} onChange={update('name')}
                            className="w-full bg-transparent outline-none text-sm text-text-dark dark:text-text-light placeholder:text-neutral-gray/60"
                        />
                    </FormField>
                    <FormField icon={<PhoneIcon weight="fill" size={15} />} label="Phone Number *">
                        <input
                ...
                <FormField icon={<EnvelopeIcon weight="fill" size={15} />} label="Email (optional)">
                    <input
                        type="email" placeholder="you@example.com"
                        value={contact.email} onChange={update('email')}
                        className="w-full bg-transparent outline-none text-sm text-text-dark dark:text-text-light placeholder:text-neutral-gray/60"
                    />
                </FormField>

                {orderType === 'delivery' && (
                    <FormField icon={<MapPinIcon weight="fill" size={15} />} label="Delivery Address *">
                        <input
                            type="text" placeholder="e.g. 14 Jungle Avenue, East Legon"
                            value={contact.address} onChange={update('address')}
                            className="w-full bg-transparent outline-none text-sm text-text-dark dark:text-text-light placeholder:text-neutral-gray/60"
                        />
                    </FormField>
                )}
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <textarea
                        ...
                        className="w-full bg-neutral-light dark:bg-brown/30 rounded-xl px-3 py-2.5 text-sm outline-none resize-none text-text-dark dark:text-text-light placeholder:text-neutral-gray/60 border border-transparent focus:border-primary/30 transition-colors"
                    />
                </div>
            </div>
...
function StepPayment({
    ...
    const methods = [
        {
            id: 'momo' as const,
            icon: <DeviceMobileIcon weight="fill" size={22} />,
            label: 'Mobile Money',
            sub: 'MTN MoMo · Telecel · AirtelTigo',
            color: 'text-warning',
        },
        {
            id: 'cash_delivery' as const,
            icon: <MoneyIcon weight="fill" size={22} />,
            label: 'Cash on Delivery',
            sub: 'Pay when your order arrives',
            color: 'text-secondary',
            hide: orderType === 'pickup',
        },
        {
            id: 'cash_pickup' as const,
            icon: <MoneyIcon weight="fill" size={22} />,
            label: 'Cash at Pickup',
            sub: 'Pay when you collect',
            color: 'text-secondary',
            hide: orderType === 'delivery',
        },
    ].filter(m => !m.hide);
    ...
    return (
        <div className="flex flex-col gap-5">
            ...
            <div className="bg-white dark:bg-brand-dark rounded-2xl p-4 shadow-sm flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center">
                        {orderType === 'delivery'
                            ? <TruckIcon weight="fill" size={18} className="text-primary" />
                            : <BagIcon weight="fill" size={18} className="text-primary" />
                        }
                    </div>
                    <div>
                        <p className="text-sm font-bold text-text-dark dark:text-text-light">
                            {orderType === 'delivery' ? 'Delivering to' : 'Picking up'}
                        </p>
                        <p className="text-xs text-neutral-gray">
                            {orderType === 'delivery' ? contact.address : 'At branch location'}
                        </p>
                    </div>
                </div>
                    ...
                    <div key={m.id}>
                        <button
                            ...
                            className={`w-full flex items-center gap-3 p-4 rounded-2xl border-2 transition-all text-left
                                ${paymentMethod === m.id
                                    ...
                                    : 'border-neutral-gray/15 hover:border-primary/30'
                                }`}
                        >
                        ...
                        {m.id === 'momo' && paymentMethod === 'momo' && (
                            <div className="mt-2 ml-4 flex flex-col gap-3 p-4 rounded-xl bg-neutral-light dark:bg-brown/30">
                                ...
                                <FormField icon={<PhoneIcon weight="fill" size={13} />} label="MoMo Number">
                                    <input
                                        type="tel" placeholder="+233 24 000 0000"
            ...
            <div className="flex gap-3">
                <button
                    onClick={onBack}
                    className="flex items-center gap-2 px-5 py-4 rounded-2xl border-2 border-neutral-gray/20 font-bold text-neutral-gray hover:border-primary/40 hover:text-primary transition-all"
                >
                ...
                <button
                    onClick={onPlace}
                    disabled={placing}
                    className="flex-1 flex items-center justify-between bg-brown dark:bg-brand-dark hover:bg-brown-light text-white font-bold px-6 py-4 rounded-2xl transition-all active:scale-[0.98] group"
                >
    ...
    return (
        <div className="flex flex-col items-center gap-6 py-8 text-center">
            ...
            <div className="bg-white dark:bg-brand-dark rounded-2xl p-5 w-full shadow-sm flex flex-col gap-3">
                ...
                <div className="flex items-center justify-between">
                    <span className="text-sm text-neutral-gray">
                        {orderType === 'delivery' ? 'Delivering to' : 'Pickup at'}
                    </span>
            ...
            <div className="bg-primary/10 border border-primary/20 rounded-2xl p-4 w-full text-sm text-text-dark dark:text-text-light">
                <p>📱 A confirmation SMS has been sent to <strong>{contact.phone}</strong> with your tracking link.</p>
            </div>
...
function FormField({ icon, label, children }: { icon: React.ReactNode; label: string; children: React.ReactNode }) {
    return (
        <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center bg-neutral-light dark:bg-brown/30 rounded-xl px-3 py-2.5 border border-transparent focus-within:border-primary/30 transition-colors">
                {children}
            </div>
        </div>
    );
            ...
            <div className="w-[95%] md:w-[85%] xl:w-[75%] max-w-5xl mx-auto">
                ...
                <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        {step !== 3 && (
                            <p className="text-sm text-neutral-gray mt-1">Complete your order details below</p>
                        )}
                    </div>
                ...
                {step === 3 ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6">
                        ...
                        <div className="lg:sticky lg:top-24 h-fit">
                            <OrderSummary orderType={orderType} paymentMethod={paymentMethod} />
                        </div>
