...
export default function AdminMenuConfigurePage() {
        ...
        <div className="max-w-5xl mx-auto px-4 md:px-8 py-6">
            ...
            {branches.length > 1 && (
                <div className="flex items-center gap-3 mb-6">
                    <label className="text-xs font-medium font-body text-neutral-gray">Branch</label>
                    <select
                        value={selectedBranchId ?? ''}
                        onChange={e => setSelectedBranchId(Number(e.target.value))}
                        className="bg-neutral-card border border-[#f0e8d8] rounded-xl px-3 py-2 text-sm font-body text-text-dark"
                    >
                        {branches.map(b => (
