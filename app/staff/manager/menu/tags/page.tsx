...
function toForm(tag?: MenuTag): TagFormState {
    if (!tag) return { name: '', slug: '', displayOrder: '0', isActive: true, ruleDescription: '' };
    return {
        id: tag.id,
        name: tag.name,
        slug: tag.slug,
...
export default function ManagerMenuTagsPage() {
    ...
    function openNew() { setEditing(toForm()); }
    function openEdit(tag: MenuTag) { setEditing(toForm(tag)); }
    ...
    async function handleToggleActive(tag: MenuTag) {
        try {
        ...
        <div className="px-4 md:px-8 py-6 max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p className="text-neutral-gray text-sm font-body mt-0.5">Manage tags and assignment rules for menu items</p>
                </div>
                <button
                    type="button"
                    onClick={openNew}
                    className="flex items-center gap-2 px-4 py-2.5 bg-primary text-brand-darker rounded-xl text-sm font-semibold font-body hover:bg-primary-hover transition-colors cursor-pointer"
                >
                    <PlusIcon size={14} weight="bold" /> New Tag
                </button>
            </div>
                ...
                {loading ? (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                            <button type="button" onClick={() => handleToggleActive(tag)} className="p-2 rounded-lg hover:bg-neutral-light cursor-pointer">
                                {tag.is_active
                            ...
                            <button type="button" onClick={() => openEdit(tag)} className="p-2 rounded-lg hover:bg-neutral-light cursor-pointer">
                                <PencilSimpleIcon size={14} />
                            </button>
                            <button type="button" onClick={() => setDeletingTag(tag)} className="p-2 rounded-lg hover:bg-error/10 text-error cursor-pointer">
                                <TrashIcon size={14} />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="bg-neutral-card rounded-2xl shadow-2xl w-full max-w-md p-6">
                        ...
                        <div className="flex flex-col gap-3">
                            <div>
                                ...
                                <input value={editing.name} onChange={e => setField('name', e.target.value)} placeholder="e.g. Popular"
                            ...
                            <div>
                                ...
                                <input value={editing.slug} onChange={e => setField('slug', e.target.value)} placeholder="e.g. popular"
                            ...
                            <div>
                                ...
                                <input type="number" min="0" value={editing.displayOrder} onChange={e => setField('displayOrder', e.target.value)}
                                    className="w-full px-3 py-2.5 bg-neutral-light border border-[#f0e8d8] rounded-xl text-sm font-body" />
                            </div>
                            <div>
                                ...
                                <textarea value={editing.ruleDescription} onChange={e => setField('ruleDescription', e.target.value)}
                                    placeholder="e.g. Ordered 50+ times in the last 30 days" rows={3}
                        ...
                        <div className="flex gap-3 mt-6">
                            <button type="button" onClick={() => setEditing(null)}
                                className="flex-1 px-4 py-2.5 border border-[#f0e8d8] rounded-xl text-sm font-medium font-body cursor-pointer hover:bg-neutral-light transition-colors">
                                Cancel
                            </button>
                            <button type="button" onClick={handleSave} disabled={saving}
                                className="flex-1 px-4 py-2.5 bg-primary text-brand-darker rounded-xl text-sm font-semibold font-body disabled:opacity-50 cursor-pointer hover:bg-primary-hover transition-colors">
                                {saving ? 'Saving…' : 'Save'}
                            </button>
                        </div>
                    </div>
            ...
            {deletingTag && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm">
                    <div className="bg-neutral-card rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
                        <div className="h-1.5 bg-error" />
                        <div className="p-6">
                            <h2 className="text-text-dark text-base font-bold font-body mb-2">Delete &ldquo;{deletingTag.name}&rdquo;?</h2>
